...
//			bottomGrid.add(wordScoreLabels[i], 2, i);
	...
	void refreshGameRoundView(GameRound twisterRound) {
		clueButtons = new Button[twisterRound.getClueWord().length()];  //initiate cluebuttons
		answerButtons = new Button[twisterRound.getClueWord().length()];//initiate answerButtons
		...
		for (int i = 0; i < twisterRound.getClueWord().length(); i++) { 
			...
			clueGrid.add(clueButtons[i], i, 1);
			clueGrid.add(answerButtons[i], i, 2);
		}
	
		
		for(int i = 0; i < answerButtons.length; i++) {
			answerButtons[i].setText(""); //clear score label
		}
